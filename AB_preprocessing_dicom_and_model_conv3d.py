...
from keras.layers import Dense, Conv3D, BatchNormalization, \
                         Flatten, MaxPooling3D
# use Theano backend
...
def build_model():
    seq = Sequential()
    seq.add(Conv3D(nb_filter=1, kernel_dim1=3, kernel_dim2=3, kernel_dim3=3,
                       input_shape=(512, 512, 512, 1), activation='relu',
                       border_mode='valid'))
    seq.add(MaxPooling3D(border_mode='valid'))
    ...
    seq.add(Dense(10, activation='tanh'))
    seq.add(Dense(10, activation='tanh'))
